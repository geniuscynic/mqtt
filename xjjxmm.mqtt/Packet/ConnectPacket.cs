...
namespace xjjxmm.mqtt.Packet
{
    internal class ConnectPacket : AbstractDataPacket<ConnectOption>
    {
        ...
        public ConnectPacket(ConnectOption connectOption)
        {
            ...
            if (connectOption.HasUserName)
            {
                _userNameByte = connectOption.UserName.ToBytes();
            }

            if (connectOption.HasPassword)
            {
                _passwordByte = connectOption.Password.ToBytes();
            }
        }
        ...
        protected override void PushVariableHeader()
        {
            ...
            if (_connectOption.CleanSession)
            {
                conectFlags |= 1 << 1;
            }
